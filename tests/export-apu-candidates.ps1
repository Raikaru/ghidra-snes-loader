...
#
# Export payload-free SNES APU/SPC interaction candidates for a private local ROM.
# The output lists only function/address metadata and APU port IDs ($2140-$2143).
#
# Usage:
#   pwsh tests/export-apu-candidates.ps1 -RomPath "C:\path\game.sfc"
...
try {
    ...
    docker cp (Join-Path $RepoRoot "tests\ExportSnesApuCandidates.java") "${Container}:$containerWork/scripts/" | Out-Null
    docker cp $RomPath "${Container}:$containerWork/rom/input.sfc" | Out-Null

    $raw = docker exec $Container bash -lc "/opt/ghidra/support/analyzeHeadless $containerWork/proj apu -import $containerWork/rom/input.sfc -scriptPath $containerWork/scripts -postScript ExportSnesApuCandidates.java -deleteProject" 2>&1
    $raw | Out-File -FilePath $headlessLog -Encoding utf8
    if ($LASTEXITCODE -ne 0) {
        throw "analyzeHeadless failed; see $headlessLog"
    }

    $jsonLine = $raw | Select-String -Pattern '^INFO  ExportSnesApuCandidates.java> JSON: ' | Select-Object -Last 1
    if (-not $jsonLine) {
